...
erDiagram
    ...
    requirements_doc {
        string id PK
        int version "vN - 정정 재생성마다 증가"
        json content "정본 구조체"
        string back_injected_from "역주입 원본 목업 (F4)"
    }
    gate_item {
        string approver_id
...
| 목업 서비스 + 역주입기 | 목업 생성·호스팅·버전 매핑, 어노테이션의 문서 흡수, 디자인 시스템 선정(테마 레지스트리) | F4 | 0 (#54 선행 — 분리 usercontent 도메인은 결정 대기 유지) |
