 ...
 */
export function RoundContext({ slots }: { slots: SlotView[] }) {
  ...
  return (
    <Card className="border-dashed">
      <CardContent className="grid gap-2 py-4 text-sm">
        <p className="font-medium">지금까지 이만큼 정리됐어요</p>
          ...
          {filled.map((slot) => (
            <li key={slot.slotKey} className="flex gap-2">
              ...
              <span>
                ...
                {slot.state === 'promoted' && <span> (개발팀 확인으로 넘김)</span>}
              </span>
            </li>
          ))}
        </ul>
        {remaining.length > 0 && (
          <p className="text-muted-foreground">
            아직 남은 것 {remaining.length}개 — {remaining.map((slot) => slot.label).join(', ')}
          </p>
        )}
      </CardContent>
    </Card>
