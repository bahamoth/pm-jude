import type { BackendResponse, BackendUsage, LlmBackend } from './backend';
...
export class LlmGateway {
  ...
  async complete<T = unknown>(promptRef: string, input: unknown): Promise<CompletionResult<T>> {
    ...
    try {
      ...
      for (let attempt = 1; attempt <= attempts; attempt++) {
        ...
        try {
          response = await this.runWithTimeout(version.body, input);
        } catch (error) {
  ...
  private runWithTimeout(promptBody: string, input: unknown): Promise<BackendResponse> {
    ...
    return new Promise<BackendResponse>((resolve, reject) => {
      ...
      this.options.backend
        .run({ promptBody, input, signal: controller.signal })
        .then(resolve, reject)
