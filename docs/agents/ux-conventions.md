...
| Content that is no longer being acted on is shown **collapsed**, not removed — and collapsed still carries an entry point to change it. | #66 shipped this as *read-only* and collapsed, reasoning that approval finished the work. #67 reversed the read-only half: the operator approved a mockup, then wanted one thing changed, and there was no door. Collapsing says "this isn't the main event"; it must not say "this can never change again". Reserve genuinely read-only for content the user could not want different. |
| Reaching a cap is a **pause**, not a wall. Say what stopped and how to continue. | #67: the mockup iteration cap counted a session's whole lifetime, so hitting it locked the screen forever. A cap should bound one round of back-and-forth, and an explicit request from the user should open a fresh one. |
| Reopening finished work must not **rewind** the journey. Add the new version; leave the completed state standing. | #67: putting the session back into `mockup` status to resume iteration would have hidden the document card and cleared the completion notice — the user asked to change one screen, not to un-approve everything. |
