...
| G-12 | 자료 첨부 수단 없음 — 요청자가 이미 정리해 둔 컨텍스트를 넣을 자리가 없어, 그 내용이 손으로 옮겨 적히거나 왕복으로 되물어진다 | F1-Attach, [ADR-0011](../adr/0011-attachment-as-clarification-input.md) | **구현** — 요청자가 입력을 넣는 모든 지점에서 자료가 발화에 붙고, 추출 텍스트가 질문 생성·완결성 판정에 실려 슬롯을 채운다. 첨부 유래 값은 출처가 기록되어 확인 화면에 표시되고(결정 8), 읽지 못한 자료는 사유와 함께 남아 여정을 멈추지 않는다(P-U3). 상태 머신은 변경 없음 — 첨부가 발화에 붙으므로 라운드 정합·왕복 상한·동시성 규칙이 그대로다. 상한 수치 3종은 결정 대기(PRD §12-19) |

G-1~G-7은 #35에서, G-9~G-11은 #44에서, G-12는 #45~#50에서 해소됐다. 리서치(#27) 발견 M-1~M-9 중 M-9(SLA 표시)는 Phase 1 게이트와 함께, 나머지는 반영됐다. 남은 미구현은 G-8(chrome 다국어 — 이연)뿐이며, §6 계약의 폭주 상한과 첨부 상한은 수치 미정(O-2)이라 계약만 서 있다.
