...
구현 결정 (#53): **기존 세션 백필 없음** — 구조체가 영속되지 않던 세션의 게시 텍스트 역파싱 백필은 신뢰할 수 없어 하지 않는다. 저장 행이 없는 레거시 세션은 조회 시 `document_delivered` 신호 수로 버전을 파생하고 화면은 게시 텍스트 파서로 폴백하며, 첫 정정 재생성부터 행이 쌓여 그때부터 저장 행이 정본이 된다.
